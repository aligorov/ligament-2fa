...
## Брендинг (платная лицензия)

Настройки → «🎨 Брендинг» — корпоративный белый лейбл: название продукта,
буква маркера, логотип (URL или `data:URI` — картинка хранится в базе и
работает без внешних хостов) и описание на странице входа. Доступен во
всех платных статусах — **демо, подписка, бессрочная**; в free интерфейс
всегда остаётся стандартным брендом Ligament. Ключ `branding`:

```json
{"name":"Ромашка SSO","mark":"R","logo":"data:image/png;base64,…","description":"Вход в корпоративный портал Ромашка"}
```

## Реклама (РСЯ) и бесплатная лицензия

Настройки → «📢 Реклама (РСЯ)»: ID блоков из кабинета
[partner.yandex.ru](https://partner.yandex.ru) (Веб-сайты → RTB-блоки) для
трёх мест — слева и справа от формы входа и в сайдбаре после входа.
Рендер через официальный загрузчик `context.js` (AdvManager), CSP
расширяется доменами Яндекса только когда реклама активна. Ключ `ads`:

```json
{"enabled":true,"blocks":{"login_left":"R-123456-1","login_right":"R-123456-2","sidebar":"R-123456-3"}}
```

Реклама показывается **только на бесплатной лицензии** (free/trial);
активация платной лицензии отключает её полностью и без рестарта.

**Direct-link сети (Monetag / Adsterra / PropellerAds).** Если домены
клиентов заранее неизвестны (self-hosted у каждого свой), РСЯ не подходит
(площадка модеруется по домену). Direct-link провайдер не требует
регистрации площадки: в кабинете сети берётся ОДНА ссылка — она работает
на любом домене. Настройки → «Реклама»: провайдер «Direct-link», URL
(и опционально надпись/картинка баннера); слоты — те же три места
(слева/справа входа, сайдбар). Ключ `ads`:

```json
{"enabled":true,"provider":"direct","direct":{"url":"https://…","label":"Реклама","image":""}}
```

**Креды РСЯ вендора по умолчанию.** Вендорская сборка может нести готовые
креды прямо в бинарнике (в публичном репозитории их нет):

```sh
ADS_CONFIG='{"enabled":true,"blocks":{"login_left":"R-123456-1","login_right":"R-123456-2","sidebar":"R-123456-3"}}'   make build    # или make docker — значение уйдёт в -ldflags и ARG
```

При первом старте такой сборки ключ `ads` засевается автоматически, пока
реклама не была настроена вручную (enabled=false и пустые ID). Клиент
всегда может переопределить значения своей админкой. ВАЖНО: oauth_token
партнёрки в `ADS_CONFIG` попадает в базу инсталляции — вшивайте его
только в сборки для собственных инсталляций; в клиентские достаточно
публичных ID блоков (они и так видны в HTML).

## Файрвол / fail2ban

Чёрные/белые списки IP — единообразно для web-интерфейса, REST API и
**RADIUS** (Access-Request с заблокированного IP получает Access-Reject
без проверки пароля). Автобан по неудачам считает IP для web и REST API;
для RADIUS брут ограничивается per-user счётчиком (см. ниже) — IP
источника RADIUS-пакетов это адрес маршрутизатора (NAS), и за NAT
неуспехи разных пользователей забанили бы Wi-Fi/VPN всем сразу.

- **Чёрный список** (`deny`) — отклонение всегда (веб 403 `ip_denied`,
  RADIUS Reject); **белый** (`allow`) — не банится автоматикой.
- **Автобан**: N неудач входа/кода (web/REST) с одного IP за окно → бан
  на срок (дефолты: 10 за 5 мин → 30 мин; ключ `fail2ban` в настройках,
  секция «🛡️ Файрвол / fail2ban»). Счётчик atomic upsert в `ip_bans`.
- **RADIUS**: неудачи не кормят IP-счётчик (самобан NAS исключён);
  перебор паролей/кодов гасит per-user блокировка — `radius.max_fail_per_user`
  за `radius.fail_window` плюс общий fail-счётчик пользователя
  (`policy.max_fail`).
- Приоритет: чёрный список → бан → белый список; IP в обоих списках
  отклоняется (fail-closed). 127.0.0.1 автоматикой не банится.
- Управление: **/admin → Файрвол** (списки, активные банки, ручной
  разбан) или API `GET/POST/DELETE /api/v1/admin/firewall*` (см. /api/docs).

Клиентский IP по умолчанию — RemoteAddr (заголовку X-Forwarded-For не
верим: подделкой обходились бы баны). Если сервер за обратным прокси
(NPM/nginx), укажите его сети в секции «🔀 Обратный прокси» (ключ
`proxy.trusted_networks`, например `172.16.0.0/12`): с доверенных
адресов принимается X-Forwarded-For — реальные внешние IP идут в аудит,
fail2ban и чёрные списки; RADIUS фильтруется по своим сокетным адресам
независимо.
...
## WPA2/WPA3-Enterprise (802.1X Wi-Fi)

RADIUS-сервер принимает **EAP-TTLS с внутренним PAP** (RFC 5281) — метод,
который умеют все распространённые supplicant'ы (Windows/macOS/Android/
Linux/iOS) и контроллеры (UniFi, MikroTik, Aruba и др.), и **PEAPv0 с
MS-CHAPv2** — дефолт нативных профилей iOS/macOS/Windows/Android без
ручной настройки. Внешний identity можно оставить анонимным
(`anonymous@…`) — реальное имя передаётся только внутри TLS-туннеля.
PAP-запросы без EAP (VPN-подключения) работают как раньше — ничего не
меняется.

**Настройка клиента (Wi-Fi профиль):**

- метод: **EAP-TTLS** (иногда «TTLS»), внутренний метод: **PAP**;
- identity: произвольный/анонимный, имя пользователя и пароль — от учётной
  записи Ligament;
- пароль = `пароль+код` из TOTP-приложения одной строкой (пароль `hunter2`
  и код `123456` → `hunter2123456`), либо пароль при push-режиме
  (`radius_push`): первое подключение ждёт push-подтверждение, повторные
  идут по окну доверия (см. ниже).

**Сертификат сервера** генерируется автоматически при первом старте:
self-signed RSA-2048 (CN=ligament, SAN=ligament, 10 лет), хранится в
настройках `radius.eap_cert` (секрет: не экспортируется и маскируется).
Клиент по умолчанию не доверяет self-signed сертификату — либо включите в
профиле «не проверять сертификат сервера», либо укажите этот сертификат
как доверенный CA (достать PEM можно из БД: таблица `settings`, ключ
`radius.eap_cert`, поле `cert_pem`), либо доверьтесь по отпечатку.

**Push по Wi-Fi — честное ограничение.** Удержание EAP-обмена до
`radius.push_wait` упирается в таймаут RADIUS на контроллере: UniFi, как
правило, даёт максимум 5 с на EAP-аутентификацию — push-подтверждение
(до 20 с и дольше) за это время не успеет. Для Wi-Fi рекомендован
TOTP-вариант «пароль+код»; push оставьте для VPN/web, где таймаут
контролируете вы (MikroTik: `/radius … timeout=30s`). Если контроллер
позволяет поднять таймаут ≥ `radius.push_wait + 5 с` — push сработает и
на Wi-Fi.

**Окно доверия (`radius.trust_days`, по умолчанию 7, `0` — выключено).**
Пароль — только первый фактор: после успешного второго фактора (push_ok
или верный TOTP-код) сервер доверяет устройству — паре «пользователь +
Calling-Station-Id» — на N дней. Повторные подключения Wi-Fi/VPN проходят
по одному паролю, без повторного 2FA (аудит `radius_trust_skip`).
Доверие хранится в таблице `radius_trusted_devices` (CSID нормализуется:
lowercase, без `:` и `-`) и никогда не заменяет пароль — MAC подделывается,
первый фактор проверяется всегда. Устройства, которые не подключались
неделю после истечения окна, подчищаются ежедневным janitor'ом.

**PEAP и нативные supplicant'ы.** В нативном PEAP-профиле (iOS/macOS/
Windows/Android) TOTP-код ввести некуда: supplicant спрашивает пароль
один раз и кэширует его. Первое подключение пользователя с выключенным
push завершится таймаутом у NAS — это ожидаемо: верный пароль сам по себе
Accept больше не даёт (закрыта дыра 1FA). Второй фактор для Wi-Fi —
push-подтверждение из приложения: после одного успешного push повторные
коннекты устройства идут по окну доверия (`radius.trust_days`) — пароль и
ничего больше.

EAP-сессии живут 90 с и резюмируются по RADIUS State; успешный Accept
несёт MS-MPPE-Send/Recv-Key (ключи WPA2/PMK-деривация на контроллере) и
обычные reply-атрибуты (`radius.reply_attributes` / per-user). Все ответы
EAP-обмена подписаны Message-Authenticator (RFC 3579). Попытки EAP-входа
пишутся в аудит как `radius_eap` (ok/fail, username, src_ip).
...
**Демо (30 дней) включается ТОЛЬКО подписанным вендором demo-файлом**
(`licgen -plan demo -days N`, ≤ 30 дней в подписи) — автоматического демо
с первого старта нет. Снос базы демо не воскрешает: файла нет → free.
Демо истекает жёстко (без grace), отзывается CRL.
