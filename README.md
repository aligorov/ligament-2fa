...
Чёрные/белые списки IP и автоблокировка по неудачам — единообразно для
web-интерфейса, REST API и **RADIUS** (Access-Request с заблокированного
IP получает Access-Reject без проверки пароля).

- **Чёрный список** (`deny`) — отклонение всегда (веб 403 `ip_denied`,
  RADIUS Reject); **белый** (`allow`) — не банится автоматикой.
- **Автобан**: N неудач входа/кода/RADIUS с одного IP за окно → бан на
  срок (дефолты: 10 за 5 мин → 30 мин; ключ `fail2ban` в настройках,
...
Linux/iOS) и контроллеры (UniFi, MikroTik, Aruba и др.). Внешний identity
можно оставить анонимным (`anonymous@…`) — реальное имя передаётся только
внутри TLS-туннеля. PAP-запросы без EAP (VPN-подключения) работают как
...
- пароль = `пароль+код` из TOTP-приложения одной строкой (пароль `hunter2`
  и код `123456` → `hunter2123456`), либо просто пароль при push-режиме
  (`radius_push` + Telegram).
